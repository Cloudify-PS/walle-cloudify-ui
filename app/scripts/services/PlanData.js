...
angular.module('cosmoUiApp').service('PlanDataConvert', function (Cosmotypesservice, $log) {
    ...
    this.edgesToBlueprint = function (data) {
        ...
        for (var i in data) {
            var edge = data[i];
            if (!result.hasOwnProperty(edge.baseType)) {
                result[edge.baseType] = [];
            }
            if(edge.baseType.search('contained') !== -1) {
                var sourceNode = findNode(result[edge.baseType], edge.baseType, edge.source);
                var targetNode = findNode(result[edge.baseType], edge.baseType, edge.target);
                if (sourceNode === null) {
                    sourceNode = {'id': edge.source, type: edge.baseType};
                }
                if (targetNode === null) {
                    targetNode = {'id': edge.target, type: edge.baseType};
                    result[edge.baseType].push(targetNode);
                }
                addChild(targetNode, sourceNode);
                if (result[edge.baseType].indexOf(sourceNode) >= 0) {
                    result[edge.baseType].splice(result[edge.baseType].indexOf(sourceNode), 1);
                }
            }
            else {
                result[edge.baseType].push(edge);
            }
        }
        return result;
    };
    ...
    this.nodesToTopology = function(data){
        ...
        function _getRelations(nodeId, type) {
            ...
            _foreach(data.edges, function (edge) {
                if ((edge.source === nodeId || edge.target === nodeId) && (type === undefined || type.indexOf(edge.baseType) > -1)) {
                    _edges.push(edge);
                }
            });
            return _edges;
        ...
        _foreach(data.nodes, function(node){
            if(_isNotType(node.type[0], noneTopologyTypes)) {
                var relations = _getRelations(node.id, ['connected', 'contained']);
    ...
    function getContainedIn(node, edges) {
        for (var x = 0; x < edges.length; x++) {
            var edge = edges[x];
            if (edge.baseType.indexOf('contained') > 0) {
    ...
    function getConnectedTo(node, edges) {
        ...
        for (var x = 0; x < edges.length; x++) {
            var edge = edges[x];
            if (edge.baseType.indexOf('connected') > 0) {
                if (edge.source === node.id) {
    ...
    this.nodesToNetworks = function (data) {
        ...
        function _getRelationsBySource(source, type) {
            var _edges = [];
            _foreach(data.edges, function (edge) {
                if (edge.source === source && (type === undefined || type.indexOf(edge.baseType) > -1)) {
                    _edges.push(edge);
                }
            });
            return _edges;
        ...
        function _getRelationsByTarget(target, type) {
            var _edges = [];
            _foreach(data.edges, function (edge) {
                if (edge.target === target && (type === undefined || type.indexOf(edge.baseType) > -1)) {
                    _edges.push(edge);
                }
            });
            return _edges;
        ...
        function _addPort(port, node) {
            ...
            var relations = _getRelationsBySource(port.id, ['depends']);
            _foreach(relations, function(relation){
                var network = _getSubnetNetworkById(relation.target);
                if(network !== null) {
                    network.devices.push({
